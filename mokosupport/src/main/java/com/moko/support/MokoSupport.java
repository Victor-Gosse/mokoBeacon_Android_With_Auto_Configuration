...
public class MokoSupport implements MokoResponseCallback {
        ...
        protected void handleMessage(MokoSupport module, Message msg) {
            switch (msg.what) {
                ...
                case HANDLER_MESSAGE_WHAT_SERVICES_DISCOVERED:
                    LogModule.i("La connexion est réussie！");
                ...
                case HANDLER_MESSAGE_WHAT_DISCONNECT:
                    ...
                    if (mBluetoothGatt != null) {
                        if (refreshDeviceCache()) {
                            LogModule.i("Vider le cache Bluetooth de la couche GATT");
                        }
                        LogModule.i("Déconnecter");
     ...
     * @Author wenzheng.liu
     * @Description Si Bluetooth est activé
     */
     ...
     * @Date 2017/5/10
     * @Author wenzheng.liu
     * @Description Que ce soit pour connecter l'appareil
     */
    ...
    public void startScanDevice(MokoScanDeviceCallback mokoScanDeviceCallback) {
        if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            LogModule.i("Lancer la numérisation de Beacon");
        }
    ...
    public void stopScanDevice() {
        if (mMokoLeScanHandler != null && mMokoScanDeviceCallback != null) {
            if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                LogModule.i("Balise de fin de balayage");
            }
            final BluetoothLeScannerCompat scanner = BluetoothLeScannerCompat.getScanner();
    ...
    public void connDevice(final Context context, final String address, final MokoConnStateCallback mokoConnStateCallback) {
        if (TextUtils.isEmpty(address)) {
            LogModule.i("connDevice: L'adresse est vide");
            return;
        }
        if (!isBluetoothOpen()) {
            LogModule.i("connDevice: Le Bluetooth n'est pas activé");
            return;
        }
        if (isConnDevice(context, address)) {
            LogModule.i("connDevice: L'appareil est connecté");
            return;
        }
        ...
        if (device != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    LogModule.i("Commencez à essayer de vous connecter");
                    mBluetoothGatt = (new BleConnectionCompat(context)).connectGatt(device, false, gattCallback);
                }
            });
        } else {
            LogModule.i("Impossible d'obtenir le périphérique Bluetooth");
        }
    }
    ...
    // Envoyer des commandes écoutables
    ...
    // Envoyer des commandes accessibles en écriture
    ...
    // Envoyer une commande inscriptible sans réponse
    ...
    // Envoyer des commandes lisibles
    ...
    // Envoyer des commandes directement (pour la mise à niveau uniquement)
    ...
    public void onCharacteristicChanged(BluetoothGattCharacteristic characteristic, byte[] value) {
        if (!mQueue.isEmpty()) {
            ...
            if (characteristic.getUuid().toString().equals(OrderType.writeAndNotify.getUuid())) {
                // Écrire une commande de notification
